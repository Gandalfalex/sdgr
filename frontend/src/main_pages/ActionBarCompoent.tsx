...
export const ActionBarComponent = () => {
    ...
    const handleMenuToggle = (pageName: string, event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    const handleLoginSignupClick = () => {
        sessionStorage.getItem("jwt") !== "None" ? sessionStorage.setItem("jwt", "None") : navigate('/login');
    };
    ...
    const createMenuItem = (page: any) => {
        ...
        if (page.type === "menu" && page.elements) {
            return (
                <div key={page.name}>
                    <Button onClick={(event) => handleMenuToggle(page.name, event)}
                            sx={{
                                my: 2,
                    ...
                    >
                        {page.elements.map((element: SubPage) => {
                            return <Tooltip title={t(element.tooltip, {ns: ['main_menu']})} key={element.name}>
                                <MenuItem
                                    onClick={() => handleNavigate(element.link, page.name)}>{t(element.name, {ns: ['main_menu']})}</MenuItem>
                            </Tooltip>
                        })}
                    </Menu>
                </div>
        ...
        <AppBar position="fixed" sx={{zIndex: theme => theme.zIndex.drawer + 1}}>
            <Toolbar>
                ...
                {sessionStorage.getItem("jwt") === "None"
                    ? <Button color="error" variant="contained" onClick={handleLoginSignupClick}>
